...
| The golden-suite port | `tests/test_golden.py` driving `tests/golden/*.txt` (vendored byte-identical from the v0.3.2 release — the pinned engine tag; the same fixtures the C smoke suite and corvid-node run) |
...
| Vendored golden fixtures (byte-identical to the v0.3.2 tag — the pinned engine version) | stable text; the suite must run offline and per-PR (verified identical to `crates/corvid-ffi/golden/`) |
